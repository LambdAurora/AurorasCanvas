	...
	public static void initModels(ModelLoadingPlugin.Context context) {
		...
		context.modifyModelOnLoad().register((model, ctx) -> {
			if (firstRun[0]) {
				...
				modelLoader.cacheAndQueueDependencies(PRESS_PLATE_MODEL_ID, pressModel);
				modelLoader.topLevelModels.put(PRESS_PLATE_MODEL_ID, pressModel);

				var screwModel = initModel(SCREW_ID, SCREW_MODEL_ID);
				modelLoader.cacheAndQueueDependencies(SCREW_MODEL_ID, screwModel);
				modelLoader.topLevelModels.put(SCREW_MODEL_ID, screwModel);
			}
